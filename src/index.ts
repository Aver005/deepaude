...
): void 
{
    ...
    const requestFromRoot = asNumberOrNull(
        (event as { request_message_id?: unknown }).request_message_id,
    );
    const responseFromRoot = asNumberOrNull(
        (event as { response_message_id?: unknown }).response_message_id,
    );
    ...
    if (event.v && typeof event.v === "object") 
{
        const nestedResponse = (
            event.v as {
                response?: { message_id?: unknown; parent_id?: unknown };
            }
        ).response;
...
async function collectDeepseekOutput(
    response: Response,
): Promise<DeepseekCollectedOutput> 
{
    let text = "";
    const idState: {
        requestMessageId: number | null;
...
function extractResponseChunks(
    event: DeepseekPatchEvent,
    state: { fragmentType: string | null },
): string[] 
{
    const chunks: string[] = [];
...
{
        const nested = event.v as {
            response?: {
                fragments?: Array<{ type?: string; content?: unknown }>;
            };
        };
        ...
        for (const fragment of event.v) 
{
            ...
            const typedFragment = fragment as {
                type?: string;
                content?: unknown;
            };
            ...
            if (
                state.fragmentType === "RESPONSE" &&
                typeof typedFragment.content === "string" &&
                typedFragment.content.length > 0
            ) 
{
    ...
    if (!body || !Array.isArray(body.messages) || body.messages.length === 0) 
{
    ...
    const deepseekResponse = await client.sendMessage(
        prompt,
    ...
    if (!deepseekResponse.ok) 
{
        return anthropicError(
            `DeepSeek upstream failed with status ${deepseekResponse.status}`,
            502,
        );
    }
    ...
    if (deepseekOutput.responseMessageId !== null) 
{
        ...
        conversation.session.setParentMessageId(
            deepseekOutput.responseMessageId,
        );
                        ...
                        for (const textChunk of splitTextForSse(text)) 
{
                    ...
                    writeSseEvent(controller, "message_stop", {
                        type: "message_stop",
                    });
                    controller.close();
                }
        ...
        if (req.method === "GET" && url.pathname === "/health") 
{
            return Response.json({
                ok: true,
                service: "anthropic-deepseek-bridge",
            });
        }
